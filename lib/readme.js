...
setTimeout(function () {
  ...
  document.querySelector('.docs-icons').innerHTML = Object.keys(GROUPS)
    .filter((grp) => !['logo', 'preview'].includes(grp))
    .map(
      (type) =>
        `<h3 data-icon>
          ${type}
  ...
  // Render preview-grid
  document.querySelector('.docs-previews').innerHTML = Object.keys(GROUPS)
    .filter((grp) => grp === 'preview')
    .map(
      (type) =>
        `<h3 data-icon>
          ${type}
...
document.addEventListener('input', function (event) {
  ...
  const previews = document.querySelector('.docs-previews')
  ...
  } else if (input.name === 'search-previews') {
    [].forEach.call(previews.children, function (el) {
      el.style.display =
        el.getAttribute('data-icon').indexOf(input.value) < 0 ? 'none' : ''
    })
  }
...
window.copyJS = function (button, jsx) {
  let name = button.dataset.id.replace(/-+./g, function (m) {
...
window.copyHTML = function (button) {
  const htmlString = document
    .querySelector(`[data-icon="${button.dataset.id}"] svg`)
    .outerHTML.trim()
