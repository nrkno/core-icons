...
setTimeout(function () {
  var bgs = {
    colorwhite: 'background: black; padding: 2px',
    colorblack: 'background: white; padding: 2px',
    blackwhite: 'background: #ccc; padding: 2px'
  }
  ;[].forEach.call(document.querySelectorAll('symbol'), function (icon) {
    ...
    group.push('<div data-icon="' + icon.id + '" style="padding:0 1em 1em 0">' +
      '<div style="font-size:' + font + 'px; ' + back + '">' +
        '<svg style="width:' + (size.width / 20) + 'em;height:' + (size.height / 20) + 'em" focusable="false" aria-hidden="true"><use xlink:href="#' + icon.id + '" /></svg>' +
       '</div>' +
      '<div style="font-size:13px">' + icon.id + '<br>' +
        '<a href="' + icon.id + '.svg" download>Download SVG</a>' +
        '<button onclick="savePDF(\'' + icon.id + '\')">Download PDF</button><br>Copy ' +
