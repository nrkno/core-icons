...
window.copyHTML = function (button) {
  ...
  copy.value = document.querySelector('[data-icon="' + button.id + '"] svg').outerHTML.trim()
  copy.select()
...
window.copyCSS = function (button) {
  var buttonText = button.textContent
  var copy = document.getElementById('docs-copy')
  var icon = document.querySelector('[data-icon="' + button.id + '"] svg').outerHTML.trim().replace(/symbol/g, 'svg').replace('<svg', '<svg xmlns="http://www.w3.org/2000/svg"')
