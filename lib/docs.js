...
;[].forEach.call(document.querySelectorAll('svg[data-core-icons] symbol'), function (icon) {
  ...
  var use = '<svg style="width:' + (box.width / 10) + 'em;height:' + (box.height / 10) + 'em" focusable="false" aria-hidden="true"><use xlink:href="#' + icon.id + '" /></svg>'
  ...
  group.push('<div data-icon="' + icon.id + '" class="' + scale + '" style="padding:0 1em 1em 0">' +
    '<div style="font-size:' + size + 'px">' + use + '</div>' +
    '<div style="font-size:13px">' + icon.id + '<br>' +
      ...
      '<input type="text" style="position:absolute;left:-999px" value="' + use.replace(/[&<>"'/]/g, (v) => ESCAPE[v]) + '">' +
    '</div>' +
  '</div>')
})
