...
setTimeout(function () {
  [].forEach.call(document.querySelectorAll('svg[data-core-icons] symbol'), function (icon) {
    ...
    group.push('<div data-icon="' + icon.id + '" class="nrk-xs-6of12 nrk-sm-4of12 nrk-md-3of12 nrk-lg-2of12" style="padding:0 1em 1em 0">' +
      '<div style="font-size:' + (icon.id.indexOf('-logo-') === -1 ? 20 : 10) + 'px">' +
      ...
      '<div style="font-size:13px">' + icon.id + '<br>' +
        ...
        '<a style="color:inherit" href="javascript:window.savePDF(\'' + icon.id + '\')">PDF</a> |' +
        '<button class="nrk-unset" style="text-decoration:underline" onclick="copyHTML(\'' + icon.id + '\')"">Copy HTML</button> |' +
        '<button class="nrk-unset" style="text-decoration:underline" onclick="copyCSS(\'' + icon.id + '\')"">Copy CSS</button>' +
      '</div>' +
    '</div>')
  })
