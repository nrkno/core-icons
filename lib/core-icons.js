...
export default function coreIcons (id) {
  ...
  const sprite = `<svg style="width:${width / 10}em;height:${height / 10}em" focusable="false" aria-hidden="true"><use xlink:href="#${id}" /></svg>`
  const svg = `<svg style="width:${width / 10}em;height:${height / 10}em" viewBox="0 0 ${width} ${height}" focusable="false" aria-hidden="true">${body}</svg>`
